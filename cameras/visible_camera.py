...
def thermal_gstreamer_pipeline(
    ...
    return f"v4l2src device=/dev/video1 ! video/x-raw,format=YUY2,width=(int){capture_width},height=(int){capture_height},framerate=(fraction){framerate}/1 ! nvvidconv ! 'video/x-raw(memory:NVMM),format=NV12' ! nvoverlaysink"
