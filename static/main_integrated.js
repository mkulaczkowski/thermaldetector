...
$(document).ready(function () {
    ...
    $('.thumbnail').click(function () {
        ...
        switch (camera) {
            case 'Front Camera':
                changeChannel('Optical');
                break;
            case 'Back Camera':
                changeChannel('Thermal');
                break;
            case 'PTZ Camera':
                changeChannel('Fusion');
                break;
        }
    });

    // Power buttons functionality
    $('#power-ptz').click(function () {
        socket.emit('cmd', { cmd: 'toggle-ptz-power' });
        alert('PTZ power toggled');
    });

    $('#power-visible-light').click(function () {
        socket.emit('cmd', { cmd: 'toggle-visible-light' });
        alert('Visible Light power toggled');
    });

    $('#power-ir-light').click(function () {
        socket.emit('cmd', { cmd: 'toggle-ir-light' });
        alert('IR Light power toggled');
    });
});
